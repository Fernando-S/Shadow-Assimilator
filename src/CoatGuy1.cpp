...
void CoatGuy::Update(float dt) {
	...
	//	Camera::Unfollow();				// Camera para de segui-los
		...
		if (!airbone && tchfloor) {
			...
			if (associated.box.x < initialX - 5 * ONETILESQUARE) {
			...
			else if (associated.box.x > initialX +  5 * ONETILESQUARE) {
		...
		if (inputManager.IsKeyDown(NUMPAD_EIGHT_KEY) && tchfloor && !airbone) {
			...
			if (inputManager.IsKeyDown(NUMPAD_FOUR_KEY) || moveEsquerda) {
				...
				speedH = { -1, 0 };
				if (oppositeSpeed > -COATGUY_SPEED) {
		...
		if ((idleL && !inputManager.IsKeyDown(NUMPAD_SIX_KEY)/* && !moveDireita*/) || (idleR && !inputManager.IsKeyDown(NUMPAD_FOUR_KEY)/* && !moveDireita*/)) {
...
void CoatGuy::Shoot(Vec2 target) {
	...
	if (facingR) {
		//associated.RemoveComponent(sprite);
		//sprite = new Sprite(associated, "./assets/img/CoatGuy/001A1BOM.png", 6, 0.2);
		//associated.AddComponent(sprite);
	}
	else if (facingL) {
		//associated.RemoveComponent(sprite);
		//sprite = new Sprite(associated, "./assets/img/CoatGuy/001A2BOM.png", 6, 0.2);
		//associated.AddComponent(sprite);
	}
	*/
