...
// Update COM BUGS, MAS ANIMACOES FUNCIONANDO
/*
void Player::Update(float dt) {
	auto inputManager = InputManager::GetInstance();
	double angleVariation = 0;
	...
	if (hp <= 0) {
		...
		if (airbone && !inputManager.IsKeyDown(W_KEY) && (Jump == 2))
			doubleJump = true; // setar para 0 quando encostar no ch�o
		...
		if (((Run == 1) && (wallAUX == 0) && (Jump == 0)) || (Ground == 1) && (Run > 5)) {
		...
		// QUEDA PRA DIREITA
		if (Fall > 0) {
			if (((Fall == 1) && (Run >= 0) && (Jump > 0)) || (Jump > 0) && (Run == 1) && (Fall > 0)) {
...
// NotifyCollision COM BUGS, MAS ANIMACOES FUNCIONANDO
/*
void Player::NotifyCollision(GameObject& other) {
	auto bullet = (Bullet*)other.GetComponent("Bullet");
	auto tile = (TileMap*)other.GetComponent("TileMap");
	...
	if (tile) {
		if (tile->colide) {
			...
			/// todo - d� ruim ao pular de lugares altos se for soh 80
			/// mas tbm d� ruim se for muito maior na hora de encostar pelo lado da plataforma
			// Colisao com chaos
			/*if ( /*(tile->GetY() <= this->associated.box.y + this->associated.box.h)
				&&/ (this->associated.box.y + this->associated.box.h <= tile->GetY() /*+ 149*//* + 90*/ /* + 120/)
				|| this->GetCenter().Distancia(Vec2(this->GetCenter().x, tile->GetY())) <= this->associated.box.h / 3
				) {
				if (!airbone && tchfloor && !SetJump) {
					verticalSpeed = 0;
					this->associated.box.y = tile->GetY() - this->associated.box.h;
